...
export class FollowingScreen extends React.Component{
    constructor(props){
        ...
        this.state = {
            email : "",name : "",user : null,userId : null,objectIdUser : null,
            following : [],search : "",waiting : true,foundUsers:[],updating : false
        }
    }
    callFunction = (uid,toUpdate) => {
        ...
        this.findUser(uid)
        var toFindFollowingOf = this.props.route.params.params.uid
        if(toFindFollowingOf != uid){
            this.findFollowers(toFindFollowingOf)
        }
    }
    componentDidMount(){
        ...
        users.on("value",data => {
          if(data.val()){
            ...
            if(matchedUser.length == 1){
                var following = matchedUser[0].followers.splice(1,matchedUser[0].following.length) 
                var updatedFollowingList = []
                following.forEach( follower => {
                  var F = firebase.database().ref("Users/" + follower.objectId)
                  F.on("value",data => {
                    if(data.val()){
                      updatedFollowingList.push(data.val())
                    }
                  })
                } )
                this.setState({ user : matchedUser[0], objectIdUser : keys[matchedIndex] ,
                     following : updatedFollowingList, followingOf : matchedUser[0],waiting : false,updating : false })
            }
          }else{
    ...
    findFollowing = uid => {
        ...
        users.on("value",data => {
          if(data.val()){
            ...
            if(matchedUser.length == 1){
                var following = matchedUser[0].following.splice(1,matchedUser[0].following.length) 
                var updatedFollowingList = []
                following.forEach( follower => {
                  var F = firebase.database().ref("Users/" + follower.objectId)
                  F.on("value",data => {
                ...
                this.setState({ following : updatedFollowingList, followingOf : matchedUser[0] })
            }
          }else{
            this.props.navigation.replace("Register")
          }
    ...
    search = Search => {
        if(Search != ""){
            ...
            var matchedUsers = this.state.following.filter(u => {
                    ...
                    >
                            ...
                            <Animated.View style={{flex : 1,marginTop : -30,marginLeft : -50}}>
                                <Avatar
                                    ...
                                    rounded
                                    showEditButton='true'
                                    source={ this.state.followingOf.userProfile ? {  uri : this.state.followingOf.userProfile }
                    ...
                    >
                        ...
                        <View style={styles.foundData}>
                            <Text style={{color : "white",fontWeight : "bold",fontSize : 20,
                                marginLeft : 30,textDecorationLine : "underline",marginBottom : 10}}>
                                Following
                            </Text>
                            { this.state.following.map( (item,index) => (
                                <ListItem 
                                    onPress={()=>{alert("ehlo")}} style={ [index == this.state.following.length - 1 ? styles.eachItem : {marginBottom : 10}] }
